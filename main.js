(async function main() {
  const imageLoadCanvas = new OffscreenCanvas(0, 0);
  const imageLoadCtx = imageLoadCanvas.getContext("2d");
  const blob = await (await fetch("brick.png")).blob();
  imageLoadCanvas.width = 16;
  imageLoadCanvas.height = 16;
  const bitmap = await createImageBitmap(blob, 0, 0, 16, 16);
  imageLoadCtx.drawImage(bitmap, 0, 0);
  const brickImageData = imageLoadCtx.getImageData(0, 0, 16, 16).data;
  ...
  for(var x = 0; x < textureSize; x++) {
    for(var y = 0; y < textureSize; y++) {
      var c = x ^ y;
      textureData.push(c * 16); // r
      textureData.push(c * 16); // g
      textureData.push(c * 16); // b
